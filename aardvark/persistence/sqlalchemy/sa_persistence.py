    ...
    def store_role_data(self, access_advisor_data: dict[str, any], session: session_type = None):
        with self.session_scope(session) as session:
            if not access_advisor_data:
                log.warning("Cannot persist Access Advisor Data as no data was collected.")
                return
        ...
        *,
        ...
        with self.session_scope(session) as session:
            ...
            try:
                ...
                if arns:
                    query = query.filter(sa_func.lower(AWSIAMObject.arn).in_([arn.lower() for arn in arns]))
            ...
            if combine and total > len(items):
                message = f"Error: Please specify a count of at least {total}."
    ...
    def get_or_create_iam_object(self, arn: str, session: session_type = None):
        with self.session_scope(session) as session:
            try:
                ...
                log.exception("failed to retrieve IAM object")
                raise
            ...
            try:
                ...
                log.exception("failed to add AWSIAMObject item to session")
                raise
            ...
            if added:
                try:
                    ...
                    log.exception("failed to create IAM object")
                    raise
            return item
