...
from aardvark import create_app, init_db
from aardvark.models.sqlalchemy.utils import teardown_db
from aardvark.updater import AccountToUpdate, persist_aa_data
...
def _prep_accounts(account_names):
    ...
    try:
        current_app.logger.info(
            "getting bucket {}".format(current_app.config.get("SWAG_BUCKET"))
        )

        swag = SWAGManager(
            **parse_swag_config_options(current_app.config.get("SWAG_OPTS"))
        )

        all_accounts = swag.get_all(current_app.config.get("SWAG_FILTER"))

        service_enabled_requirement = current_app.config.get(
            "SWAG_SERVICE_ENABLED_REQUIREMENT", None
        )
    ...
    for name in account_names:
        if name not in lookup:
            current_app.logger.warn("Could not find an account named %s" % name)
            continue
