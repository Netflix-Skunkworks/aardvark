...
import aardvark.manage
import aardvark.persistence.sqlalchemy
...
ARTIFACT_ARCHIVE_DIR = (
    os.environ.get("SWAG_CONFIG_TEST_ARCHIVE_DIR") or DEFAULT_ARTIFACT_ARCHIVE_DIR
)
COMMAND_ARCHIVE_DIR = (
    os.environ.get("SWAG_CONFIG_TEST_COMMAND_ARCHIVE_DIR") or ARTIFACT_ARCHIVE_DIR
)
...
# in setUpClass().
DEFAULT_PARAMETERS = dict([(k, v["default"]) for k, v in CONFIG_OPTIONS.items()])
...
def default_db_uri():
    ...
    return "{localdb}:///{path}/{filename}".format(
        localdb=aardvark.manage.LOCALDB,
        path=os.getcwd(),
        filename=aardvark.manage.DEFAULT_LOCALDB_FILENAME,
    )
...
class TestConfigBase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        DEFAULT_PARAMETERS_NO_SWAG["db_uri"] = CONFIG_OPTIONS["db_uri"]["default"]
    ...
    def call_aardvark_config(
        ...
        self.last_config_command = "{} {}".format(
            command,
            get_config_option_string(cmdline_option_spec, short_flags=short_flags),
        )

        self.commands_issued.append(self.last_config_command)
        spawn_config = pexpect.spawn(self.last_config_command)

        self.conduct_config_prompt_sequence(spawn_config, input_option_spec)
    ...
    def conduct_config_prompt_sequence(self, spawned, input_option_spec):
        ...
        control_prompts = [(pexpect.EOF, "eof"), (pexpect.TIMEOUT, "timeout")]
        config_option_prompts = [
            (v["config_prompt"], k) for k, v in CONFIG_OPTIONS.items()
        ]
        expect_prompts = [
        ...
        response_spec["use_swag"] = "y" if "swag_bucket" in input_option_spec else "N"
...
class TestConfigNoPrompt(TestConfigBase):
    ...
    def test_no_prompt_all_parameters(self):
        ...
        self.case_worker(cmdline_option_spec=cmdline_option_spec,)
    ...
    def test_no_prompt_all_parameters_short(self):
        ...
        self.case_worker(cmdline_option_spec=cmdline_option_spec, short_flags=True)
...
class TestConfigPrompt(TestConfigBase):
    ...
    def test_prompted_all_cmdline_parameters(self):
        ...
        self.case_worker(cmdline_option_spec=cmdline_option_spec, prompt=True)
    ...
    def test_prompted_no_swag(self):
        ...
        self.case_worker(input_option_spec=input_option_spec, prompt=True)
